...
def train_model():
    ...
    for i, (key, value) in enumerate(trains.items()):
        X_train += [word_tokenize(v, format="text") for v in value
                    ] + [convert_to_no_accents(v) for v in value]
...
classes = pickle.load(open("src/pkl/classes.pkl", "rb"))
...
vectorizer = pickle.load(open("src/pkl/tfidf_vectorizer.pkl", "rb"))
